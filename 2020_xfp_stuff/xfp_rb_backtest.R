...
# TODO - SOME DERRICK HENRY STRANGENESS SOMEWHERE
xfp_rushes_2019 <- pbp_df_2019 %>%
  ...
  summarize(carries = n(),
            ...
            actual_hPPR_fp = sum(yards_gained * 0.1) + sum(touchdown * 6)
  ) %>%
  filter(carries >= 50)
...
short_xfp_targets_2019 <- avg_exp_fp_df %>%
  select(receiver, exp_hPPR_pts, hPPR_pts, games) %>%
  summarize(player = receiver, games, exp_hPPR_pts, hPPR_pts)

short_xfp_rushes_2019 <- xfp_rushes_2019 %>%
  summarize(player = rusher, total_rush_xfp, actual_rush_fp = actual_hPPR_fp)

xfp_rb <- merge(short_xfp_rushes_2019, short_xfp_targets_2019, by="player") %>%
  mutate(
    total_xfp = total_rush_xfp + exp_hPPR_pts,
    total_xfp_pg = total_xfp / games,
    total_actual_fp = hPPR_pts + actual_rush_fp
  )

fit <- lm(total_xfp ~ total_actual_fp, data = xfp_rb)
summary(fit)
