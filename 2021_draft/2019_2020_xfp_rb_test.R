...
# 2019 pbp data
pbp_df_2019 <- readRDS(url('https://raw.githubusercontent.com/guga31bb/nflfastR-data/master/data/play_by_play_2019.rds'))

## 2019 Expected

xfp_rushes_2019 <- pbp_df_2019 %>%
...
short_xfp_targets_2019 <- avg_exp_fp_df %>%
...
short_xfp_rushes_2019 <- xfp_rushes_2019 %>%
...
xfp_rb <- merge(short_xfp_rushes_2019, short_xfp_targets_2019)
...
xfp_rb <- mutate(xfp_rb,
                 ...
                 total_xfp_pg_2019 = total_xfp / games
) %>%
  filter(games >= 3) # TODO - TEMP FIX FOR THE GAMES PLAYED ISSUES

short_xfp_rb <- xfp_rb %>%
  select(player_name = player, player_id = gsis_id, total_xfp_pg_2019)

## 2020 Actual

stats_2020 <- nflfastR::load_pbp(2020) %>%
  dplyr::filter(season_type == "REG") %>%
  nflfastR::calculate_player_stats() %>%
  dplyr::mutate(hPPR_pts = ((rushing_yards + receiving_yards) * 0.1) + ((rushing_tds + receiving_tds) * 6) + (receptions * 0.5) + ((rushing_fumbles_lost + receiving_fumbles_lost) * -2)) %>%
  dplyr::mutate(hPPR_pts_pg_2020 = hPPR_pts / games)

merge <- merge(short_xfp_rb, stats_2020)

ggplot(merge, aes(x=total_xfp_pg_2019, y=hPPR_pts_pg_2020, label=player_name)) +
  geom_point() +
  geom_text_repel() +
