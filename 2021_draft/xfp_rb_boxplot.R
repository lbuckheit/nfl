...
xfp_rushes_2020 <- pbp_df_2020 %>%
  ...
  group_by(rusher, rusher_id, game_id) %>%
...
avg_exp_fp_df <- pbp_df_2020 %>%
  ...
  ungroup %>%
  group_by(posteam, receiver, receiver_id, game_id) %>%
  # Summary columns
  summarize(
...
short_xfp_targets_2020 <- avg_exp_fp_df %>%
  select(game_id, receiver, receiver_id, games, exp_hPPR_pts, hPPR_pts) %>%
  summarize(player = receiver, game_id, gsis_id = receiver_id, rec_games = games, exp_hPPR_pts, actual_catch_fp = hPPR_pts)

relevant_players <- short_xfp_targets_2020 %>%
  group_by(gsis_id) %>%
  summarize(total_xfp = sum(exp_hPPR_pts)) %>%
  filter(total_xfp > 100)

merge = merge(short_xfp_targets_2020, relevant_players)
...
ggplot(merge, aes(x=reorder(player, -total_xfp), y=exp_hPPR_pts, label=player)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = -90)) +
  labs(x = "Player",
       y = "exp_hPPR_pts_pg",
       title = "2020 Expected hPPR Pts. Boxplots",
