...
short_xfp_wr_2019 <- short_xfp_targets_2019 %>%
  select(player_name = player, player_id = gsis_id, exp_hPPR_pts, rec_games) %>%
  mutate(exp_hPPR_pts_pg_2019 = exp_hPPR_pts / rec_games) %>%
  filter(exp_hPPR_pts_pg_2019 > 5)
...
stats_2020 <- nflfastR::load_pbp(2020) %>%
  ...
  dplyr::mutate(hPPR_pts = ((receiving_yards) * 0.1) + (receiving_tds * 6) + (receptions * 0.5) + (receiving_fumbles_lost) * -2) %>%
  dplyr::mutate(hPPR_pts_pg_2020 = hPPR_pts / games)

merge <- merge(short_xfp_wr_2019, stats_2020)

ggplot(merge, aes(x=exp_hPPR_pts_pg_2019, y=hPPR_pts_pg_2020, label=player_name)) +
  geom_point() +
  geom_text_repel() +
